...
def main(_argv):
    ...
    while True:
        ...
        bbox_num = valid_detections[0]
        worker_count = 5
        print(bbox_num, worker_count, bbox_num//worker_count )
        distribution = bbox_num//worker_count
        print(0,worker_count*distribution, distribution + 1)
        lock = threading.Lock()
        for i in range(0,worker_count*distribution, distribution + 1):
            start = i
            end = i + worker_count
            print(f'{start}:{end}')
            t = threading.Thread(target=add_bboxes_to_tracker,
             args=(boxes, image_h, image_w, multitracker, start, end,tracker_type, frame,lock))
            threads.append(t)
            t.start()
        ...
        _, new_boxes = multitracker.update(frame)
        ...
        image = utils.draw_bbox_tracker(frame, pred_bbox)
        curr_time = time.time()
...
def add_bboxes_to_tracker(boxes, image_h, image_w, multitracker, start, end, tracker_type, frame, lock):   
    results = []
    #print(boxes[0][start:end])

    for bbox in boxes[0][start:end]:
        ...
        if (coor[0]> 0 and coor[1]> 0 and coor[2]> 0 and coor[3] > 0 and coords[1] < 680):
            results.append([createTrackerByName(tracker_type), frame, coords])
        with lock:
            print('Lock adquire for {results}')
            for result in results:
                multitracker.add(result[0],result[1],result[2])
